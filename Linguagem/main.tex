...
\lstdefinestyle{slstyle}{
    ...
    language=C,
    morekeywords={*, sequence, instrument, time, performance, in, on, play, always, at, and, except, after, sequentially, times, loop, number},
...
Um marco de tempo é uma variável que referencia um dado momento entre o início e o fim, inclusive, da peça musical. \texttt{start} é um marco de tempo pré-definido, representando o início da peça. 
\subsubsection{Operador de duração}
O operador \texttt{|}\textit{x}\texttt{|}, aceita um parâmetro (\textit{x}), do tipo sequence ou performance, e devolve um time igual à sua duração.
\begin{lstlisting} 
...
time chorusStart = start + 2*|verse|;
time otherTimeChorusStarts = start + 3.14*|verse|;
...
É também possível definir novos instrumentos à custa de instrumentos já existentes, e associar nomes aos tons de um instrumento.

\begin{lstlisting} 
// duplicar um instrumento ja existente
instrument percussion: drums;

// mapear os tons do instrumento (associar nomes aos tons de um instrumento)
instrument percussion: AcousticBassDrum = B0,
                       BassDrum1 = C1,
                       SideStick = C#1,
                       AcousticSnare = D1,
                       HandClap = D#1,
